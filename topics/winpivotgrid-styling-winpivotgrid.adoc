...
AppStylist at runtime requires adding the link:infragistics4.win.appstylistsupport.v{ProductVersion}~infragistics.win.appstyling.runtime.appstylistruntime_members.html[AppStylistRuntime] control on to the form and calling a method, in code, to start up the AppStylist when the application starts running. The following two steps demonstrates this:
