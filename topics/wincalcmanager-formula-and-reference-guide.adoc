...
* The integer indexes in references are by default the visible row indexes. There is a FormulaRowIndexSource property off of the link:infragistics4.win.ultrawingrid.v{ProductVersion}~infragistics.win.ultrawingrid.ultragridoverride.html[UltraGridOverride] object that controls whether these indexes are row indexes, visible row indexes or list indexes.
