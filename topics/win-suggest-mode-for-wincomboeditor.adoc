...
Using the new  pick:[win-forms="link:infragistics4.win.ultrawineditors.v{ProductVersion}~infragistics.win.ultrawineditors.ultracomboeditor~autocompletemode.html[AutoCompleteMode]"]  property, you can select between three types of suggest modes.
...
Not only did we implement similar functionality on WinComboEditor, but it's associated embeddable editor,  pick:[win-forms="link:infragistics4.win.v{ProductVersion}~infragistics.win.editorwithcombo.html[EditorWithCombo]"] . This allows you to use the suggest feature in WinCombo™, as well as WinGrid™ columns. This enables you to create more sophisticated drop-down lists. 
