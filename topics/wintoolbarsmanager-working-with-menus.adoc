...
Menus in WinToolbarsManager™ are conceptually the same as standard .NET menus and Microsoft Office menus. In the .NET menu architecture, a menu is a special MenuItem that contains a collection of other MenuItems. Similarly, in UltraToolbarsManager a Menu is a special Tool represented by the  pick:[win-forms="link:infragistics4.win.ultrawintoolbars.v{ProductVersion}~infragistics.win.ultrawintoolbars.popupmenutool.html[PopupMenuTool]"]  class - one of the 12 native tool types supported by UltraToolbarsManager. The PopupMenuTool has the ability to contain other tools (including other PopupMenuTools).
