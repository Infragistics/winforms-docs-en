...
[NOTE]
====
We set the WinGrid's   pick:[win-forms="link:infragistics4.win.v{ProductVersion}~infragistics.win.ultracontrolbase~useappstyling.html[UseAppStyling]"]  property to False, because if you are using Application Styling when the customer makes a selection the setting will have no effect while UseAppStyling is True.
====
