...
Gesture support for the  _WinDayView_   control consists of  *Vertical*  and  *Horizontal*  panning (See link:touch-gestures-overview.html#_Ref349131108[Panning Gestures]) in the link:{ApiPlatform}win.ultrawinschedule.v{ProductVersion}~infragistics.win.ultrawinschedule.ultradayview~timeslots.html[TimeSlots] area and the link:{ApiPlatform}win.ultrawinschedule.v{ProductVersion}~infragistics.win.ultrawinschedule.appointment~alldayevent.html[AllDayEvent] area. The implementation also includes time slots drag-selection and appointments dragging operations.
...
[NOTE]
====
The height of the header elements for the day ( link:{ApiPlatform}win.ultrawinschedule.v{ProductVersion}~infragistics.win.ultrawinschedule.dayofmonth~daynumber.html[DayNumber]) is not increased.
====
...
Except link:{ApiPlatform}win.ultrawinschedule.v{ProductVersion}~infragistics.win.ultrawinschedule.calendarcombo.datebutton_members.html[DateButton] element,  _WinCalendarCombo_   exposes the same elements as the  _WinMonthViewMulti_   illustrated above.
