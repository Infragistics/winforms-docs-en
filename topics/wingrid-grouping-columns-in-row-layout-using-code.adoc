...
Grouping Columns is possible in Row Layout mode by setting the WinGrid control’s  pick:[win-forms="link:infragistics4.win.ultrawingrid.v{ProductVersion}~infragistics.win.ultrawingrid.ultragridband~rowlayoutstyle.html[RowLayoutStyle]"]  property to GroupLayout. This allows for an indefinite number of groupings.
