...
Enabling the Touch functionality occurs at the form level rather than the control level as it is unlikely that both Touch and Non-Touch system use the same application. Adding the link:{ApiPlatform}win.v{ProductVersion}~infragistics.win.touch.ultratouchprovider.html[UltraTouchProvider] component to the form enables Form level touch support for  _Infragistics_   controls.
