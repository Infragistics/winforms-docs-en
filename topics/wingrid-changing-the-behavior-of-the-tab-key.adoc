...
The default behavior for the WinGrid™ is for the TAB key to move focus from one cell to the next. You can cause a TAB keystroke to move to the next control instead of the next cell by using the  pick:[win-forms="link:infragistics4.win.ultrawingrid.v{ProductVersion}~infragistics.win.ultrawingrid.ultragridlayout~tabnavigation.html[TabNavigation]"]  property of the  pick:[win-forms="link:infragistics4.win.ultrawingrid.v{ProductVersion}~infragistics.win.ultrawingrid.ultragridlayout.html[DisplayLayout]"]  object.
