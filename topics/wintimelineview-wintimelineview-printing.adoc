...
When printing WinTimelineView™ content, you can use the control’s link:infragistics4.win.ultrawinschedule.v{ProductVersion}~infragistics.win.ultrawinschedule.ultrascheduleprintdocument~initializetimelineview_ev.html[InitializeTimelineView] event to make any adjustments that you feel are necessary for the printed output display. This event is invoked when the control is created during the print operation.
