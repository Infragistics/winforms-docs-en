...
When WinFontNameEditor™ displays its list of fonts, it defaults to displaying each font name formatted using the font itself. However, you can show the font names in the standard font instead. This feature is controlled by the  pick:[win-forms="link:infragistics4.win.ultrawineditors.v{ProductVersion}~infragistics.win.ultrawineditors.ultrafontnameeditor~showfontnamesinfont.html[ShowFontNamesInFont]"]  property.
