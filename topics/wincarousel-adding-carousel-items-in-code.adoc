...
This topic demonstrates how to add link:{ApiPlatform}win.ultrawincarousel.v{ProductVersion}~infragistics.win.ultrawincarousel.ultracarousel_members.html[UltraCarousel] items using the Code-Behind.
...
In this topic, you will learn how to add the UltraCarousel control and configure it through the control’s properties in code. The control has a link:{ApiPlatform}win.ultrawincarousel.v{ProductVersion}~infragistics.win.ultrawincarousel.ultracarousel~datasource.html[DataSource] property that can be used to bind any collection of strings or images that would display on the items.
...
An Item in the UltraCarousel control is the visual representations of the data object. Items appear as square tiles on the path. The link:{ApiPlatform}win.ultrawincarousel.v{ProductVersion}~infragistics.win.ultrawincarousel.carouselpath~itemslots.html[ItemSlots] property determines the number of items ( _Default=5_  ) displayed on the path. The default size of each item is 64x64 (In pixels).
...
This code demonstrates binding the control to a data using its link:{ApiPlatform}win.ultrawincarousel.v{ProductVersion}~infragistics.win.ultrawincarousel.ultracarousel~datasource.html[DataSource] property.
