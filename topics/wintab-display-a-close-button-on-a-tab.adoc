...
WinTab, WinTabStrip, and WinTabbedMdiManager all include a close button which you can display on each tab. With the close button, your end user can close individual tabs without leaving the bounds of the tab itself – they can also click the tab with the middle mouse button to close it. By default, the close button is hidden, so if you want to expose the button to your end user, you'll have to set the  pick:[win-forms="link:infragistics4.win.ultrawintabcontrol.v{ProductVersion}~infragistics.win.ultrawintabcontrol.ultratabcontrolbase~closebuttonlocation.html[CloseButtonLocation]"]  property to Tab.
