...
The unit of time for the duration can be set using the link:infragistics4.win.ultrawinschedule.v{ProductVersion}~infragistics.win.ultrawinschedule.task~setduration(timespan,timespanformat).html[SetDuration] method, which accepts a TimeSpanFormat parameter.

A link:infragistics4.win.ultrawinschedule.v{ProductVersion}~infragistics.win.ultrawinschedule.ultracalendarinfo~taskdurationworkingtimeperday.html[TaskDurationWorkingTimePerDay] property is exposed by the UltraCalendarInfo object, which provides a way to specify the number of working hours in a workday.

The link:infragistics4.win.ultrawinschedule.v{ProductVersion}~infragistics.win.ultrawinschedule.calendardatesettings.html[CalendarDateSettings] collection object provides ways to customize the workday status and working hours for a particular date.
