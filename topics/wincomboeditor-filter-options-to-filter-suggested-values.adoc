...
[NOTE]
====
By default when the AutoSuggestFilterMode is set to Contains, the character string typed by the end user during an auto-suggest session will be highlighted within each item in the drop down. This setting can be altered using the link:infragistics4.win.v{ProductVersion}~infragistics.win.valuelist~formatfiltereditems.html[FormatFilteredItems] property.
====
