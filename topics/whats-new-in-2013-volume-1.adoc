...
The  _WinLabel_   control now supports a new link:{ApiPlatform}win.misc.v{ProductVersion}~infragistics.win.misc.ultralabel~autoellipsis.html[AutoEllipsis] property that detects and handles those scenarios where the text exceeds the control’s bounds.
