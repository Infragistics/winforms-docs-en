...
This topic demonstrates the operations you can perform with Matrices. It covers link:{ApiPlatform}math.v{ProductVersion}~infragistics.math.matrix.html[Matrix] instances only, but the information in it applies to all matrix types – link:{ApiPlatform}math.v{ProductVersion}~infragistics.math.matrix.html[Matrix], link:{ApiPlatform}math.v{ProductVersion}~infragistics.math.booleanmatrix.html[BooleanMatrix], and link:{ApiPlatform}math.v{ProductVersion}~infragistics.math.complexmatrix.html[ComplexMatrix].
...
The link:{ApiPlatform}math.v{ProductVersion}~infragistics.math.matrix.html[Matrix] class also supports arithmetical operations between a matrix and a complex number.
...
If you want true matrix multiplication, use the link:{ApiPlatform}math.v{ProductVersion}~infragistics.math.compute~matrixproduct.html[Compute.MatrixProduct] method instead of the multiplication shown here.
...
The <, $$<=$$, >, and >= operators compare the corresponding values in the matrices. These operators return a link:{ApiPlatform}math.v{ProductVersion}~infragistics.math.booleanmatrix.html[BooleanMatrix] instance and are defined only for link:{ApiPlatform}math.v{ProductVersion}~infragistics.math.matrix.html[Matrix] and link:{ApiPlatform}math.v{ProductVersion}~infragistics.math.complexmatrix.html[ComplexMatrix]. Each element of the returned Boolean matrix will be the result of the comparison of the corresponding elements from the initial matrices.

link:{ApiPlatform}math.v{ProductVersion}~infragistics.math.matrix.html[Matrix] and link:{ApiPlatform}math.v{ProductVersion}~infragistics.math.complexmatrix.html[ComplexMatrix] can be compared to each other and to *double* and link:{ApiPlatform}math.v{ProductVersion}~infragistics.math.complex.html[Complex]. link:{ApiPlatform}math.v{ProductVersion}~infragistics.math.booleanmatrix.html[BooleanMatrix] can be compared also to a Boolean value.
