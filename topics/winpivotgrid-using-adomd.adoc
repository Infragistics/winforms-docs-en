...
ADOMD data provider requires a connection via link:{ApiPlatform}olap.dataprovider.adomd.v{ProductVersion}~infragistics.olap.adomd.adomdinitialsettings~connectionstring.html[ConnectionString] property, in order to communicate with the data in the server.

The following table lists parameters that are necessary to establish a connection to the data in the server using the link:{ApiPlatform}olap.dataprovider.adomd.v{ProductVersion}~infragistics.olap.adomd.adomdinitialsettings_members.html[AdomdInitialSettings] object. There are other optional parameters for authentication and security (not listed here). The binding code example below contains all these settings.
