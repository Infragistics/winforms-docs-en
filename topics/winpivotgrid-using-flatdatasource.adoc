...
This topic explains how to use the link:{ApiPlatform}olap.datasource.flat.v{ProductVersion}~infragistics.olap.flatdata.flatdatasource_members.html[FlatDataSource] class for displaying  _IEnumerable_   data in the  _WinPivotGrid_™.
...
When a FlatDataSource is instantiated with a reference to the `List<SportsData>` and calls the link:{ApiPlatform}olap.datasource.flat.v{ProductVersion}~infragistics.olap.flatdata.flatdatasource~generatecube.html[GenerateCube] method, the following sequence of events take place:

1. A list of link:{ApiPlatform}olap.datasource.flat.v{ProductVersion}~infragistics.olap.flatdata.olapaxiselementdescriptorbase~propertydescriptor.html[PropertyDescriptor] objects describing the publicly exposed properties for that class is obtained via the `TypeDescriptor.GetProperties` method.

2. The property descriptor list is iterated.

3. Each property of a numeric type (in this example, the “Budget” and “Players” members) triggers creation of a link:{ApiPlatform}olap.datasource.flat.v{ProductVersion}~infragistics.olap.flatdata.measuredescriptor_members.html[MeasureDescriptor], which defines the schematic attributes of the resulting Measures which can appear on the measures slice of the cube.

4. Each property of a non-numeric type (in this example, the “Nation”, “City” and “Sport” members) triggers creation of a link:{ApiPlatform}olap.datasource.flat.v{ProductVersion}~infragistics.olap.flatdata.hierarchydescriptor_members.html[HierarchyDescriptor], which defines the schematic attributes of the resulting Hierarchies which can appear on the row, column, or filter dimensions.

5. For each link:{ApiPlatform}olap.datasource.flat.v{ProductVersion}~infragistics.olap.flatdata.hierarchydescriptor_members.html[HierarchyDescriptor] a corresponding link:{ApiPlatform}olap.datasource.flat.v{ProductVersion}~infragistics.olap.flatdata.leveldescriptor_members.html[LevelDescriptor] objects of the `HierarchyDescriptor` are generated.
