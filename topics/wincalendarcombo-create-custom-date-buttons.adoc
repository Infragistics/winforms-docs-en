...
You may find it necessary when using the WinCalenderCombo™ to add a  pick:[win-forms="link:infragistics4.win.ultrawinschedule.v{ProductVersion}~infragistics.win.ultrawinschedule.calendarcombo.datebutton.html[DateButton]"]  that performs some custom action when clicked. For example, when the button is clicked it makes the selected date two days from the current date. The following example code shows how you could implement this functionality.
