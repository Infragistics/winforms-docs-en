...
[start=4]
. *Add to Dictionary* -- Clicking Add to Dictionary will add the currently misspelled word to the user's dictionary. This button is only available when the  pick:[win-forms="link:infragistics4.win.ultrawinspellchecker.v{ProductVersion}~infragistics.win.ultrawinspellchecker.ultraspellchecker~userdictionary.html[UserDictionary]"]  property is set.
[start=5]
