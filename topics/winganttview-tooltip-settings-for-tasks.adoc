...
This topic demonstrates the various tooltip customization options available for tasks displayed in the WinGanttView™ control. Tooltips are displayed for tasks that are visible in the chart area of the control. The tooltip settings exist within the link:infragistics4.win.ultrawinganttview.v{ProductVersion}~infragistics.win.ultrawinganttview.ultraganttview~timelinesettings.html[TimelineSettings] object of the control. For more information on adding tasks to GanttView please see the link:winganttview-adding-tasks-to-winganttview.html[Adding Tasks to WinGanttView] topic.
...
The link:infragistics4.win.ultrawinganttview.v{ProductVersion}~infragistics.win.ultrawinganttview.tasktooltipdisplayingeventargs.html[TaskToolTipDisplaying] event can be fired to add tooltips to Tasks.
