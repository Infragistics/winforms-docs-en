...
1. Click on the tile and select link:{ApiPlatform}win.ultrawinlivetileview.v{ProductVersion}~infragistics.win.ultrawinlivetileview.livetile~culturespecificviews.html[CultureSpecificViews] using the  *Properties*  grid.
...
6. Change the current link:{ApiPlatform}win.ultrawinlivetileview.v{ProductVersion}~infragistics.win.ultrawinlivetileview.culturespecificlivetileview~culturename.html[CultureName] to “`ja-JP`”, or you can also use your computer’s regional settings to change the culture, resulting in this result.
