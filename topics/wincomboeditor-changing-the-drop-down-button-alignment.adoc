...
You can change the location of the drop-down arrow on WinComboEditor™ from its default right location. To make such a change all you need to do is change the  pick:[win-forms="link:infragistics4.win.ultrawineditors.v{ProductVersion}~infragistics.win.ultrawineditors.ultracomboeditor~dropdownbuttonalignment.html[DropDownButtonAlignment]"]  property off the WinComboEditor, to one of the available  pick:[win-forms="link:infragistics4.win.v{ProductVersion}~infragistics.win.editorwithcombo~buttonalignment.html[ButtonAlignment]"]  enumerations.
