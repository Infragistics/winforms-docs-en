...
* link:infragistics4.win.v{ProductVersion}~infragistics.win.layout.gridbagconstraint.html[GridBagConstraint] -- Contains several useful properties to manage where a control is placed in the grid bag layout, as well as how many virtual rows or columns it spans.
* link:infragistics4.win.misc.v{ProductVersion}~infragistics.win.misc.controllayoutitem~includeinlayout.html[IncludeInLayout] -- Determines whether or not the control is included in the grid bag layout (i.e., whether or not the control is managed by WinGridBagLayoutPanel).
* link:infragistics4.win.misc.v{ProductVersion}~infragistics.win.misc.controllayoutitem~preferredsize.html[PreferredSize] -- WinGridBagLayoutPanel will attempt to resize the control based on this value. The PreferredSize property is ignored if the GridBagConstraint.Fill property is set.