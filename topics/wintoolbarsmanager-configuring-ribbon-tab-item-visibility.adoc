...
This feature allows developers to configure the visibility of tab items using the link:{ApiPlatform}win.ultrawintoolbars.v{ProductVersion}~infragistics.win.ultrawintoolbars.ribbon~tabitemareadisplaystyle.html[TabItemAreaDisplayStyle] property in  _UltraToolbarsManager_  ’s Ribbon.

[[_Ref376430820]]

=== Configuring Tab item visibility

The visibility of the tab item area can be configured to show or hide tab items. Hiding tab items will also reduce the height of the ribbon area. The available options from the link:{ApiPlatform}win.ultrawintoolbars.v{ProductVersion}~infragistics.win.ultrawintoolbars.ribbon~tabitemareadisplaystyle.html[TabItemAreaDisplayStyle] configurable property are listed in the following table:
