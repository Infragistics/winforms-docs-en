...
This topic explains how to use the link:infragistics4.olap.datasource.flat.v{ProductVersion}~infragistics.olap.flatdata.flatdatasource_members.html[FlatDataSource] data adapter classes to easily bind the WinPivotGrid™ control to data obtained from a database, using either SQL Server or OLEDB technology.
