...
[start=7]
. Inside the ShowFormulaBuilderDialog method declare a new  pick:[win-forms="link:{ApiPlatform}win.ultrawincalcmanager.v{ProductVersion}.formulabuilder~infragistics.win.ultrawincalcmanager.formulabuilder.functionbuilderdialog.html[FormulaBuilderDialog]"] , passing in the IForumlaProvider variable that was passed into the ShowFormulaBuilderDialog.
[start=8]
