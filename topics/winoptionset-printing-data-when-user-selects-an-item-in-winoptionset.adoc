...
To write code for handling changes to the WinOptionSet™, use the  pick:[win-forms="link:infragistics4.win.ultrawineditors.v{ProductVersion}~infragistics.win.ultrawineditors.ultraoptionset~valuechanged_ev.html[ValueChanged]"]  event to listen for when the user selects a new item in the set. For example, the following code would print out the index of the item and the value of the item that was just selected.
